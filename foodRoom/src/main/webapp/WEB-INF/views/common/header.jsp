					...
					<td class="dropdown">		
						...
						<div class="infoView">
							<c:if test="${fr_class == 13}">
								<a href="${contextPath}/myPage.do"><input type="button" value="내정보" class=""/></a>
								<a href="${contextPath}/storeViewListForm.do"><input type="button" value="에약하기" class=""/></a>
								<a href=""><input type="button" value="에약내역" class=""/></a>
							...
							<c:if test="${fr_class == 12}">
								<a href="${contextPath}/myPage.do"><input type="button" value="내정보" class=""/></a>
								<a href="${contextPath}/reservation.do"><input type="button" value="에약내역" class=""/></a>
								<a href=""><input type="button" value="업체관리" class=""/></a>
								<a href="${contextPath}/logout.do" onclick="return confirm('정말 로그아웃 하시겠습니까?');"><input type="button" value="로그아웃" class=""/></a>
							</c:if>
							<c:if test="${fr_class == 02}">
								<a href="${contextPath}/myPage.do"><input type="button" value="내정보" class=""/></a>
								<a href="${contextPath}/regiForm.do"><input type="button" value="업체등록" class="" id="regiFormBtn"/></a>
								<a href="${contextPath}/logout.do" onclick="return confirm('정말 로그아웃 하시겠습니까?');"><input type="button" value="로그아웃" class=""/></a>
							</c:if>
							<c:if test="${fr_class == 11}">
